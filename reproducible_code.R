...
met_df <- met_df %>%
  ...
  mutate(
    ...
    hypoxia_group_descriptive_cox = ifelse(HYPOXIA_SCORE > median(HYPOXIA_SCORE, na.rm = TRUE), "High", "Low"),
    hypoxia_group_descriptive_cox = factor(hypoxia_group_descriptive_cox, levels = c("High", "Low"))
  )
...
save_model_plan(met_df, "METABRIC_hypoxia_only", c("hypoxia_group_descriptive_cox"), diag_dir)
m_met_base <- run_cox_model(
  df = met_df,
  rhs_formula = "hypoxia_group_descriptive_cox",
  required_vars = c("hypoxia_group_descriptive_cox"),
  model_name = "METABRIC_hypoxia_only",
  out_dir = table_dir,
  relevel_list = list(hypoxia_group_descriptive_cox = "High")
)
...
save_model_plan(met_subtyped, "METABRIC_subtype_adjusted", c("hypoxia_group_descriptive_cox", "SUBTYPE_ANY"), diag_dir)
m_met_subtype <- run_cox_model(
  df = met_subtyped,
  rhs_formula = "hypoxia_group_descriptive_cox + SUBTYPE_ANY",
  required_vars = c("hypoxia_group_descriptive_cox", "SUBTYPE_ANY"),
...
build_preferred_row <- function(label, cox_model, cox_term_pattern, ph_term_exact, aft_model = NULL) {
  ...
  if (ph_ok || is.null(aft_model)) {
    cox_note <- if (ph_ok) {
      "PH diagnostics acceptable; report Cox HR."
    } else {
      "PH diagnostics suggested possible non-proportionality; Cox HR is reported descriptively with caution."
    }
...
preferred_results_df <- bind_rows(
  ...
  build_preferred_row("METABRIC hypoxia-only", "METABRIC_hypoxia_only", "^hypoxia_group", "hypoxia_group_descriptive_cox", "METABRIC_hypoxia_only_AFT"),
...
ph_flag_df <- if (!is.null(ph_df_all)) {
  key_map <- tibble::tribble(
    ...
    "METABRIC hypoxia-only", "METABRIC_hypoxia_only", "hypoxia_group_descriptive_cox",
